...
public class TcpClient {
    ...
    public static void main(String[] args) throws Exception{
        ...
        String host =  "192.168.0.191";            // DEFAULT_HOST;
        int port = 9761;//DEFAULT_PORT;
        if (args.length > 0) {
            host = args[0];
        }
        if (args.length > 1) {
        ...
        BufferedReader readerlan=new BufferedReader(new InputStreamReader(socket.getInputStream()));
        String ln = null;
        try {
            while ((ln = reader.readLine()) != null) {
                System.out.println(readerlan.toString());
                writer.write(ln );
